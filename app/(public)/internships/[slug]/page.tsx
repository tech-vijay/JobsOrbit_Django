...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!opportunity) {
    return buildMetadata({
      ...
      description: "The requested internship opportunity could not be found.",
    });
  }
  ...
  return buildMetadata({
    title: opportunity.seoTitle || `${opportunity.title} Internship at ${companyName}`,
    description:
      opportunity.seoDescription ||
      `Apply for ${opportunity.title} Internship at ${companyName}. ${opportunity.description.slice(0, 140)}...`,
    keywords: opportunity.seoKeywords || [opportunity.title, companyName, "student internship", siteConfig.name.toLowerCase()],
    path: `/internships/${opportunity.slug}`,
  });
}
